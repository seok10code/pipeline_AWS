...
conf = SparkConf()
conf.set("spark.executor.memory", "20g")  # 각 Executor에 4GB의 메모리 할당
conf.set("spark.driver.memory", "4g")    # Driver에 4GB의 메모리 할당
conf.set("spark.sql.shuffle.partitions", "200")  # 파티션 수를 200으로 설정
...
path2 = "s3://dataproject-bucket/table/carmore_all_table_final_small_size.csv"
tableN = spark.read.option("header", "true").option("inferSchema", "true").csv(path1)
db_tables=tableN.select("database","table").collect()

#테이블 스키마 담겨있는 csv 파일 
allSchemaDf = spark.read.option("header","true").option("inferschema", "true").csv("s3://dataproject-bucket/table/COLUMNS_202306090813.csv") 
...
for dbtb in db_tables:  
    ...
    tinyint_col_list = tableSchemaDf.filter(col('DATA_TYPE') == 'tinyint').select('COLUMN_NAME').rdd.flatMap(lambda x:x).collect() ## tinyint
    ...
    jdbcDF.repartition(300)
    jdbcDF.write.format('delta').option("mergeSchema", "true").mode('overwrite').save(f"s3://dataproject-bucket/data/{database_name}_test/{table_name}/") ## 테이블 저장
