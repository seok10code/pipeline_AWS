...
def query_formating_function(dbname, tbname, all_list, datetime_list, date_list, time_list): ##RDS에 jDBC 쿼리 포메팅 함수
...
for dbtb in db_tables:  
    ...
    if not jdbcDF.rdd.isEmpty():
        try:
            ...
            jdbcDF.write.format('delta').mode('overwrite').save(f"s3://dataproject-bucket/data/{database_name}/{table_name}/") ## 테이블 저장
