...
jdbcDF.printSchema()
print(jdbcDF.select("wari_driver_gender","wari_api", "wari_driver2_gender").show())
...
spark.sql(f"DROP TABLE IF EXISTS {database_name}_delta.{table_name}")
...
## Glue Catalog 저장 
spark.sql(f"""
CREATE EXTERNAL TABLE IF NOT EXISTS {database_name}_delta.{table_name}({result_schem})
